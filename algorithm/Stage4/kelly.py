"""Kelly criterion position sizing for arbitrage bets.

The Kelly criterion tells you the optimal fraction of your bankroll to
wager given an edge and the odds.  For prediction-market arbitrage the
"bet" is: enter a spread position and wait for convergence.

Full Kelly is famously aggressive, so we default to half-Kelly
(KELLY_FRACTION = 0.5 in config) for practical use.
"""

from __future__ import annotations
from backend.common.config import KELLY_FRACTION, MAX_POSITION_FRACTION
...
def kelly_size(
    ...
    reward_per_dollar: estimated capturable arbitrage amount per $ risked
    risk_per_dollar  : estimated adverse move amount per $ risked
    match_quality    : semantic / structural match quality in [0, 1]

    Returns
    -------
    (kelly_f, position_usd)
        kelly_f      : the fractional Kelly allocation (before bankroll)
        position_usd : dollar amount to deploy
    """
    ...
    # Fold in a conservative fee/slippage haircut.
    fee_estimate = 0.02
