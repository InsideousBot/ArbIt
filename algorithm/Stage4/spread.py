"""Spread computation utilities for Stage 4.

The spread is the difference between the YES price on one platform and
the implied YES price (1 − NO price) on the other.  A positive spread
means we can buy cheap on one side and sell dear on the other.
"""

from __future__ import annotations
from backend.common.models import MatchedPair, Direction
...
def compute_spread(pair: MatchedPair) -> tuple[float, Direction]:
    ...
    the spread is 0.07 and the direction is BUY_B_SELL_A (buy the cheap
    side on B, sell the expensive side on A).
    """
...
def two_sided_spread(pair: MatchedPair) -> float:
    ...
    (which is the same as selling YES there), you lock in profit when
    the combined cost < 1.00.
    ...
    cost_1 = price_b + pair.market_a.no_price  # buy YES@B + NO@A
