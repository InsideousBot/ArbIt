    ...
    for sig in signals:
        ...
        if res_a and res_b:
            if res_a == res_b:
                ...
                # Markets resolved differently → pair was not truly equivalent
                # Record as UNKNOWN rather than booking a large synthetic loss
                outcome = "UNKNOWN"
                realized_pnl = None
...
def run_simulation(req: SimRunRequest) -> Dict[str, Any]:
    """Trigger (or replay) the backtest simulation and return aggregate results."""
    try:
        db = get_db()
        trades = _build_simulation_trades(db)
        ...
        return {
            ...
            "final_capital": round(req.initial_capital + total_pnl, 2),
